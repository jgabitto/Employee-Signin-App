...
const resultsDate = document.getElementById('1');
const resultsFName = document.getElementById('2');
const resultsLName = document.getElementById('3');
const resultsEmail = document.getElementById('4');
const resultsTimeIn = document.getElementById('5');
const resultsTimeOut = document.getElementById('6');
const resultsHoursWorked = document.getElementById('7');
const resultsDateFF = document.getElementById('1FF');
const resultsFNameFF = document.getElementById('2FF');
const resultsLNameFF = document.getElementById('3FF');
const resultsEmailFF = document.getElementById('4FF');
const resultsDateInFF = document.getElementById('5FF')
const resultsDateOutFF = document.getElementById('6FF')
const resultsTimeInFF = document.getElementById('7FF');
const resultsTimeOutFF = document.getElementById('8FF');
const resultsHoursWorkedFF = document.getElementById('9FF');
const amTimeOutBtn = document.getElementById('amTimeOut');
const pmTimeOutBtn = document.getElementById('pmTimeOut');
const amTimeInBtn = document.getElementById('amTimeIn');
const pmTimeInBtn = document.getElementById('pmTimeIn');
...
function checkBrowser() {
    ...
    if (firefoxAgent) {
        timeIn.classList.add('hideBtn');
        timeOut.classList.add('hideBtn');
        timeInFirefox.classList.remove('hideBtn');
        timeOutFirefox.classList.remove('hideBtn');
        dateInI.classList.remove('hideBtn');
        dateOutI.classList.remove('hideBtn');
        dateIn.classList.remove('hideBtn');
        dateOut.classList.remove('hideBtn');
        dateInH.classList.remove('hideBtn');
        dateOutH.classList.remove('hideBtn');
    }
}
...
editBtn.addEventListener('click', async (e) => {
    ...
    if (fName.readOnly) {
        ...
        if (firefoxAgent) {
            ...
            if (timeInFirefox.readOnly) {
                timeInFirefox.removeAttribute('readonly');
                timeOutFirefox.removeAttribute('readonly');
            }
        } 
    } else {
        ...
        if (firefoxAgent) {
            ...
            if (!timeInFirefox.readOnly) {
                timeInFirefox.setAttribute('readonly', true);
                timeOutFirefox.setAttribute('readonly', true);
            }
        }
    }
})
...
submitBtn.addEventListener('click', async (e) => {
    ...
    try {    
        ...
        if (firefoxAgent) {
            const timeInSplitFirefox = calculateTimeFirefox(timeInFirefox);
            const timeOutSplitFirefox = calculateTimeFirefox(timeOutFirefox);
            ...
            data = {
                ...
                'timeIn': `${timeInSplitFirefox[0]}:${timeInSplitFirefox[1]}`,
                'timeOut': `${timeOutSplitFirefox[0]}:${timeOutSplitFirefox[1]}`,
            ...
            data = {
                ...
                'lastName': formData.get('lName'),
                'email': formData.get('email'),
                'timeIn': `${timeInSplit[0]}:${timeInSplit[1]}:${timeInSplit[2]}`,
                'timeOut': `${timeOutSplit[0]}:${timeOutSplit[1]}:${timeOutSplit[2]}`,
                'hoursWorked': diffTime(timeInSplit[3], timeOutSplit[3])
                };  
        }
        ...
        if (json.status === 'Success') {
            if (firefoxAgent) {
                ...
                resultsDateFF.innerHTML = json.data.date;
                resultsFNameFF.innerHTML = json.data.firstName;
                resultsLNameFF.innerHTML = json.data.lastName;
                resultsEmailFF.innerHTML = json.data.email;
                resultsDateInFF.innerHTML = json.data.dateIn;
                resultsDateOutFF.innerHTML = json.data.dateIn;
                resultsTimeInFF.innerHTML = json.data.timeIn;
                resultsTimeOutFF.innerHTML = json.data.timeOut;
                resultsHoursWorkedFF.innerHTML = json.data.hoursWorked;
            } else {
                firstRow.classList.add('hideBtn');
...
// Extract date, day and time from timeIn and timeOut
function calculateTime(timeEle) {
    console.log(timeEle.value);
    let dateSplit = timeEle.value.split('-');
    let daySplit = dateSplit[2].split('T');
    let timeSplit = daySplit[1].split(':');
    ...
    timeSplit = [years, months, days, hours, minutes];

    return timeSplit;
}

dateIn.addEventListener('change', () => {
    const elementFirefox = calculateDateFirefox(dateIn);
    const years = elementFirefox[0];
    const months = elementFirefox[1];
    const days = elementFirefox[2];

    timeInFirefox.removeAttribute('readOnly');

    nextDay = ((parseInt(elementFirefox[2]) + 1).toString()).padStart(2, '0');
...
dateOut.addEventListener('change', () => {
    const elementFirefox = calculateDateFirefox(dateOut);
    const years = elementFirefox[0];
    const months = elementFirefox[1];
    const days = elementFirefox[2];
    timeOutFirefox.removeAttribute('readOnly');
...
timeInFirefox.addEventListener('change', () => {
    const elementFirefox = calculateTimeFirefox(timeInFirefox);
    const hours = elementFirefox[0];
    const minutes = elementFirefox[1];

    timeOutFirefox.setAttribute("min", `${elementFirefox[0]}:${elementFirefox[1]}`);
    timeOutFirefox.setAttribute("max", `T00:00`);

    if (timeInFirefox.value && timeOutFirefox.value) {
...
timeOutFirefox.addEventListener('change', () => {
   if (timeInFirefox.value && timeOutFirefox.value) {
       calculateHoursWorkedFirefox();
   } else {
       hoursWorked.value = '';
...
function calculateHoursWorkedFirefox() {
    const start = calculateTimeFirefox(timeInFirefox);
    const end = calculateTimeFirefox(timeOutFirefox);
...
function calculateDateFirefox(dateEle) {
...
function calculateTimeFirefox(timeEle) {
